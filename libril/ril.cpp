...
static struct unsol_conv unsolResponse_conv[] = {
    ...
    { RIL_UNSOL_ON_SS_V6, RIL_UNSOL_ON_SS },
    { RIL_UNSOL_STK_CC_ALPHA_NOTIFY_V6, RIL_UNSOL_STK_CC_ALPHA_NOTIFY },
                                ...
                                size_t datalen)
{
    ...
    for (int i = 0; i < (int)NUM_ELEMS(unsolResponse_conv); i++) {
        if (unsolResponse_conv[i].v6 == unsolResponse) {
            unsolResponse = unsolResponse_conv[i].curr;
            RLOGD("RIL_onUnsolicitedResponse %d -> %d",
                unsolResponse_conv[i].v6, unsolResponse);
